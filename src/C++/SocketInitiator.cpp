...
// OMD_THIRD_PARTY_CHANGE: Added getSessionID function to let collector know session id in onDisconnect function
bool SocketInitiator::getSessionID(SessionID* session_id, int s)
{
	const SocketConnections::iterator i = m_connections.find( s );
	const SocketConnections::iterator j = m_pendingConnections.find( s );

	SocketConnection* pSocketConnection = NULL;
	if(i != m_connections.end())
	{
		pSocketConnection = i->second;
	}
	...
	*session_id = pSocketConnection->getSession()->getSessionID();
	return true;
}
