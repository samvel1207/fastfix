...
namespace FIX
{
	...
	class Message : public FieldMap
	{
		...
		// OMD_THIRD_PARTY_CHANGE: added this usings since this class is overriding one of isSetField and getField functions which hides the second ones in the base class
	public:
		using FieldMap::isSetField;
		using FieldMap::getField;
	...
	public:
		...
		// OMD_THIRD_PARTY_CHANGE: override this functions in order to search/get field in both maps
		virtual bool isSetField(int field) const
		{
		...
		virtual const std::string& getField(int field) 
		{
			try
			{
				return FieldMap::getField(field);
			}
			catch (const FieldNotFound& ex)
			{
