	...
	void Session::nextResendRequest(const Message& resendRequest, const UtcTimeStamp& timeStamp)
	{
		...
		for (i = messages.begin(); i != messages.end(); ++i)
		{
			...
			if (m_sessionID.isFIXT())
			{
				...
				if (strMsgType.empty())
					pMsg.reset(new Message(*i, sessionDD, applicationDD, m_validateLengthAndChecksum, m_noDataFields));
				else
				{
					...
					pMsg.reset(new Message(hdrOrder, trlOrder, msgOrder, *i, sessionDD, applicationDD, m_validateLengthAndChecksum, m_noDataFields));
				}
			}
			else
			{
				if (strMsgType.empty())
					pMsg.reset(new Message(*i, sessionDD, m_validateLengthAndChecksum, m_noDataFields));
				else
				{
					const message_order& hdrOrder = sessionDD.getHeaderOrderedFields();
					const message_order& trlOrder = sessionDD.getTrailerOrderedFields();
					const message_order& msgOrder = sessionDD.getMessageOrderedFields(strMsgType);
					pMsg.reset(new Message(hdrOrder, trlOrder, msgOrder, *i, sessionDD, m_validateLengthAndChecksum, m_noDataFields));
				}
			}
